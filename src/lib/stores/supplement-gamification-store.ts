...
const LEVEL_THRESHOLDS = [
	{
		level: 1,
		xp: 0,
		name: "Supplement Curious",
		unlockedFeatures: ["Basic supplement browser"],
	},
	{
		level: 2,
		xp: 500,
		name: "Knowledge Seeker",
		unlockedFeatures: ["Advanced search filters", "Basic quiz challenges"],
	},
	{
		level: 3,
		xp: 1200,
		name: "Supplement Student",
		unlockedFeatures: ["Stack builder", "Progress tracking"],
	},
	{
		level: 4,
		xp: 2500,
		name: "Wellness Scholar",
		unlockedFeatures: ["Social features", "Advanced analytics"],
	},
	{
		level: 5,
		xp: 4500,
		name: "Supplement Expert",
		unlockedFeatures: ["Expert challenges", "Mentorship opportunities"],
	},
	{
		level: 6,
		xp: 7500,
		name: "Master Educator",
		unlockedFeatures: ["Create custom content", "Advanced social features"],
	},
	{
		level: 7,
		xp: 12000,
		name: "Wellness Legend",
		unlockedFeatures: ["All premium features", "Recognition badges"],
	},
];
...
const DEFAULT_SUPPLEMENT_ACHIEVEMENTS: SupplementAchievement[] = [
	...
	},
	{
		...
		name: "Safety First",
		description:
			"Learn all about supplement interactions and contraindications",
		icon: "shield",
		category: "learning",
		xpReward: 300,
		unlocked: false,
...
export const useSupplementGamificationStore =
	create<SupplementGamificationState>()(
		devtools(
			persist(
				(set, get) => ({
					// Initial state
					...
					updateStreak: () => {
						...
						if (lastDate === yesterdayStr) {
							...
							if (
								newStreak === 7 &&
								!state.unlockedAchievements.includes("week-warrior")
							) {
								get().unlockAchievement("week-warrior");
							}
							if (
								newStreak === 30 &&
								!state.unlockedAchievements.includes("adherence-champion")
							) {
					...
					recordSupplementLearning: (supplementId) => {
						...
						const newSupplementsLearned = [
							...state.supplementsLearned,
							supplementId,
						];
						...
						if (
							newTotal === 1 &&
							!state.unlockedAchievements.includes("first-supplement")
						) {
							get().unlockAchievement("first-supplement");
						}
						if (
							newTotal >= 20 &&
							!state.unlockedAchievements.includes("evidence-expert")
						) {
					...
					recordCategoryExploration: (category) => {
						...
						// Check achievements
						if (
							newCategoriesExplored.length >= 12 &&
							!state.unlockedAchievements.includes("category-explorer")
						) {
					...
					shareRecommendation: () => {
						...
						if (
							newCount >= 10 &&
							!state.unlockedAchievements.includes("social-contributor")
						) {
