...
const SupplementHistorySchema = new Schema<ISupplementHistory>({
  ...
  medicineSystem: {
    type: String,
    enum: ["TCM","AYURVEDA","GREEK_ROMAN","EUROPEAN_HERBALISM","MODERN_SCIENCE","OTHER"],
    required: true,
    index: true,
  },
  ...
  relatedSupplements: [{ type: Schema.Types.ObjectId, ref: "ComprehensiveSupplement", index: true }],
...
SupplementHistorySchema.index({ title: "text", polishTitle: "text", description: "text", polishDescription: "text" });
// Timeline queries by medicine system and start year
SupplementHistorySchema.index({ medicineSystem: 1, eraStartYear: 1 });

const SupplementHistory: Model<ISupplementHistory> = mongoose.models.SupplementHistory || mongoose.model<ISupplementHistory>("SupplementHistory", SupplementHistorySchema);
