				...
				() => {
					...
					try {
						...
						if (validationErrors.length > 0) {
							throw new Error(
								`Błędy walidacji: ${validationErrors.map((e) => e.message).join(", ")}`,
							);
						}
						...
						if (Math.random() < 0.1) {
							// 10% chance of calculation error
							throw new Error(
								"Błąd podczas obliczania dawki - nieprawidłowe parametry suplementu",
							);
						}
						...
						const result: DosageCalculationResult = {
							...
							warnings: [],
							polishWarnings: [],
							recommendations: input.supplements.map(
								(s) => `Rekomendacja dla suplementu ${s.supplementId}`,
							),
							polishRecommendations: input.supplements.map(
								(s) => `Zalecenie dla suplementu ${s.supplementId}`,
							),
						};
...
export function useDosageCalculation() {
	...
	const calculateDosage = async (
		...
		try {
			...
			const errorMessage =
				error instanceof Error ? error.message : "Nie udało się obliczyć dawek";
			...
			if (errorMessage.includes("walidacji")) {
				...
				errorHandler.handleDosageError(
					"Obliczenia przekroczyły limit czasu. Spróbuj ponownie.",
				);
			} else {
