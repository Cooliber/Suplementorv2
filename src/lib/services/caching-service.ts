...
class CachingService {
  ...
  get<T>(key: string): { data: T | null; status: CacheStatus; source: string } {
    ...
    if (key.startsWith('supplements:')) {
      try {
        ...
        console.warn('Failed to retrieve cache entry from localStorage:', error);
      }
    }
  ...
  private async processOfflineQueue(): Promise<void> {
    ...
    for (const item of itemsToProcess) {
      try {
        ...
        item.retryCount++;
        console.warn(`Failed to sync item ${item.id}, retry ${item.retryCount}:`, error);
  ...
  private cleanup(): void {
    ...
    while (remainingEntries.length > 0 && this.getTotalSize() > this.MAX_CACHE_SIZE * 0.8) {
      ...
      if (key.startsWith('supplements:')) {
        try {
          ...
          console.warn('Failed to remove old cache entry from localStorage:', error);
        }
      }
    }
  }
