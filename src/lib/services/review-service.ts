...
export interface ReviewData {
  ...
  effectiveness: "very_effective" | "effective" | "neutral" | "ineffective" | "very_ineffective";
  sideEffects?: string[];
...
class ReviewService {
      ...
      setTimeout(() => {
        ...
        try {
          ...
          const result: ReviewSubmissionResult = {
            ...
            message: "Opinia została wysłana pomyślnie i oczekuje na moderację",
  ...
  async getSupplementReviews(supplementId: string, limit = 10, offset = 0): Promise<ReviewDisplayData[]> {
    try {
      // Simulate API call
      ...
      const mockReviews: ReviewDisplayData[] = [
        {
          ...
          content: "Bardzo polecam ten suplement. Działa zgodnie z oczekiwaniami i nie powoduje skutków ubocznych.",
        ...
        },
        {
          ...
          content: "Suplement działa, ale efekty nie są tak spektakularne jak się spodziewałem.",
  ...
  async deleteReview(reviewId: string): Promise<boolean> {
    try {
      ...
      if (Math.random() < 0.05) { // 5% chance of deletion error
...
export function useReviewSubmission() {
  ...
  const submitReview = async (review: ReviewData): Promise<ReviewSubmissionResult | null> => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : "Nie udało się wysłać opinii";

      // Handle specific error types
      if (errorMessage.includes("walidacji")) {
        errorHandler.handleValidationError(errorMessage, "Sprawdź poprawność wprowadzonych danych");
      } else if (errorMessage.includes("czas")) {
        errorHandler.handleReviewError("Przekroczono limit czasu. Spróbuj ponownie.");
      } else {
...
export function useSupplementReviews(supplementId: string) {
  ...
  const fetchReviews = async (limit = 10, offset = 0) => {
    ...
    try {
      const service = ReviewService.getInstance();
      const data = await service.getSupplementReviews(supplementId, limit, offset);
      setReviews(data);
      loadingState.stopLoading();
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : "Nie udało się pobrać opinii";
      errorHandler.handleReviewError(errorMessage);
      loadingState.setLoadingError(errorMessage);
    }
  };
