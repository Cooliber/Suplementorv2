...
export const SupplementFormSchema = z.object({
	...
	polishName: z
		...
		.regex(
			polishTextRegex,
			"Polska nazwa może zawierać tylko polskie litery, spacje i myślniki",
		)
		.optional(),
	...
	form: z.enum(
		["tabletki", "kapsułki", "proszek", "płyn", "krople", "żel", "inne"],
		{
	...
	recommendedDosage: z
		...
		.refine(
			...
			"Nieprawidłowy format dawki (np. '500mg dziennie')",
		),
	...
	availability: z
		.enum(["dostępny", "niedostępny", "wycofany"])
		.default("dostępny"),
	...
	imageUrl: z.string().url("Nieprawidłowy adres URL obrazka").optional(),
});
...
export const EnhancedReviewSchema = z.object({
	...
	pros: z
		.array(
			z
				.string()
				.min(1, "Zaleta nie może być pusta")
				.max(200, "Zaleta nie może być dłuższa niż 200 znaków"),
		)
	...
	cons: z
		.array(
			z
				.string()
				.min(1, "Wada nie może być pusta")
				.max(200, "Wada nie może być dłuższa niż 200 znaków"),
		)
	...
	polishPros: z
		.array(
			z
				.string()
				.min(1, "Polska zaleta nie może być pusta")
				.max(200, "Polska zaleta nie może być dłuższa niż 200 znaków"),
		)
	...
	polishCons: z
		.array(
			z
				.string()
				.min(1, "Polska wada nie może być pusta")
				.max(200, "Polska wada nie może być dłuższa niż 200 znaków"),
		)
	...
	sideEffects: z
		.array(
			z
				.string()
				.min(1, "Efekt uboczny nie może być pusty")
				.max(200, "Efekt uboczny nie może być dłuższy niż 200 znaków"),
		)
...
export const validateSupplementInteraction = (
	supplements: string[],
): {
	isValid: boolean;
	warnings: string[];
	...
	if (
		supplements.includes("melatonina") &&
		supplements.includes("dziurawiec")
	) {
...
export const getFieldValidationMessage = (
	...
	const polishMessages: Record<string, string> = {
		"Nazwa suplementu musi mieć co najmniej 2 znaki":
			"Nazwa suplementu musi mieć co najmniej 2 znaki",
		"Nazwa suplementu nie może być dłuższa niż 100 znaków":
			"Nazwa suplementu nie może być dłuższa niż 100 znaków",
		"Nazwa może zawierać tylko litery, cyfry, spacje i myślniki":
			"Nazwa może zawierać tylko litery, cyfry, spacje i myślniki",
		...
		"Wybierz prawidłową formę suplementu":
			"Wybierz prawidłową formę suplementu",
		"Zalecana dawka jest wymagana": "Zalecana dawka jest wymagana",
		"Nieprawidłowy format dawki":
			"Nieprawidłowy format dawki (np. '500mg dziennie')",
...
// UserProfile schema for dosage calculator (duplicate removed - using the main one above)
