...
export interface AtomSubscription {
  atomId: string;
  events: (AtomUpdate | AtomValidationResult) => void;
...
export interface AtomAnalytics {
  ...
  trends: {
    ...
    userEngagement: Array<{
      action: string; count: number; timestamp: string;
    }>;
  };
}
...
export interface UserAtomInteraction {
  ...
  atomId: string;
  interaction: {
    type: 'view' | 'expand' | 'share' | 'cite' | 'reference' | 'bookmark' | 'note';
    timestamp: string;
    duration?: number;
    context?: string;
  };
...
export type UseAtomReturn = {
  ...
  validate: () => AtomValidationResult | void;
  bookmark: () => void;
  share: () => void;
  cite: () => string;
};
...
export const precisionRequirements = {
  ...
  neuroimaging: {
    resolution: 'high' | 'medium' | 'standard',
    fieldStrength: { minimum: 1.5, preferred: 3.0 },
    sequences: ['T1', 'T2', 'EPI', 'FLAIR', 'REST', 'DWI', 'fMRI']
  },
  biochemical: {
    concentrations: { 
