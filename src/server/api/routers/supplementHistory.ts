...
export const supplementHistoryRouter = createTRPCRouter({
  getAll: publicProcedure
    .input(z.object({ limit: z.number().min(1).max(100).optional(), skip: z.number().min(0).optional() }).optional())
  ...
  getByMedicineSystem: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (input.eraStartFrom != null || input.eraStartTo != null) {
        q.eraStartYear = {};
        if (input.eraStartFrom != null) q.eraStartYear.$gte = input.eraStartFrom;
  ...
  getTimeline: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      return ctx.db.supplementHistory
        .find(match)
        .sort({ eraStartYear: 1 })
        .select({ id: 1, title: 1, polishTitle: 1, era: 1, eraStartYear: 1, eraEndYear: 1, medicineSystem: 1, tags: 1 })
        .lean();
    }),
