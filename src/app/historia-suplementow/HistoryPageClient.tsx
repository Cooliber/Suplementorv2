...
import { MedicineSystemFilter, type MedicineSystem } from "@/components/history/MedicineSystemFilter";
import { SupplementTimeline } from "@/components/history/SupplementTimeline";
import { HistoryEntryCard, type HistoryEntry } from "@/components/history/HistoryEntryCard";
import { Input } from "@/components/ui/input";
import { Button } from "@/components/ui/button";
import { Search, X } from "lucide-react";

export function HistoryPageClient({ initialEntries }: { initialEntries: HistoryEntry[] }) {
  const router = useRouter();
  const searchParams = useSearchParams();
  const [searchQuery, setSearchQuery] = React.useState("");
  const [filteredEntries, setFilteredEntries] = React.useState(initialEntries);

  const currentSystem = (searchParams.get("system") ?? "ALL") as MedicineSystem | "ALL";

  const handleSystemChange = React.useCallback((system: MedicineSystem | "ALL") => {
    ...
    router.push(`/historia-suplementow?${params.toString()}`, { scroll: false });
  }, [router, searchParams]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <MedicineSystemFilter value={currentSystem} onChange={handleSystemChange} />
