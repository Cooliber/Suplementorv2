...
export const metadata: Metadata = {
	...
	description:
		"Oś czasu tradycyjnych systemów medycznych (TCM, Ajurweda i inne) – przegląd kluczowych wydarzeń i źródeł.",
};

export default async function Page({
	searchParams,
}: { searchParams?: { system?: MedicineSystem | "ALL" } }) {
	const system = (searchParams?.system ?? "ALL") as MedicineSystem | "ALL";
	const all = await api.supplementHistory.getTimeline(
			...
			<div className="container mx-auto space-y-6 px-4 py-8">
				<header className="space-y-2">
					...
					<p className="text-muted-foreground">
						Przegląd rozwoju tradycyjnych systemów medycznych i farmakologii
						roślinnej.
					</p>
				</header>
