...
import { type ExportFile, ExportFileManager } from "./ExportFileManager";
import { ExportPreview } from "./ExportPreview";
import { type ExportTemplate, ExportTemplates } from "./ExportTemplates";
...
export const ExportManager: React.FC<ExportManagerProps> = ({
	...
	const [activeTab, setActiveTab] = useState<"main" | "templates" | "files">(
		"main",
	);
	...
	const handleExport = useCallback(
		async (exportData: ExportableData[], config: ExportConfiguration) => {
			setIsExporting(true);
	...
	const handlePreview = useCallback(
		(previewData: ExportableData[], config: ExportConfiguration) => {
	...
	const handleTemplateSelect = useCallback(
		(template: ExportTemplate) => {
			const configWithTemplate: ExportConfiguration = {
				...
				selectedColumns:
					template.recommendedColumns || defaultConfig.selectedColumns,
				...
				try {
					if (navigator.share) {
						...
						await navigator.clipboard.writeText(
							file.downloadUrl || window.location.href,
						);
					}
				} catch (error) {
					console.error("Sharing failed:", error);
	...
	const handleExportWithFileTracking = useCallback(
		async (exportData: ExportableData[], config: ExportConfiguration) => {
		...
		<div className={cn("space-y-6", className)}>
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						{description ||
							`${data.length} rekordów • ${dataTypeInfo.description}`}
					</CardDescription>
				</CardHeader>
				<CardContent>
					<div className="flex items-center justify-between">
						<div className="flex items-center gap-4">
							<Badge variant="outline">{data.length} rekordów</Badge>
						...
						<ExportDialog
							...
							savedConfigurations={savedConfigurations.map(
								...
								>
									<div className="flex-1">
										...
										<div className="text-muted-foreground text-sm">
											...
											{config.lastUsed &&
												` • Ostatnio użyte: ${config.lastUsed.toLocaleDateString("pl-PL")}`}
										</div>
									</div>
									<div className="flex items-center gap-2">
										<Badge variant="outline">Użyć: {config.useCount}</Badge>
