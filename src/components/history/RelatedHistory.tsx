...
async function RelatedHistoryContent({ supplementMongoId }: { supplementMongoId: string }){
  const entries = await api.supplementHistory.getRelatedHistory({ supplementMongoId });
  ...
  return (
    <Card>
      ...
      <CardContent className="grid grid-cols-1 gap-4 md:grid-cols-2">
        {entries.map((e)=> (
          <HistoryEntryCard key={e.id} entry={{
            id: e.id,
            title: e.title,
...
export default function RelatedHistory({ supplementMongoId }: { supplementMongoId?: string }){
