...
export type HistoryEntry = {
  ...
  eraStartYear: number;
  eraEndYear: number;
  medicineSystem: "TCM"|"AYURVEDA"|"GREEK_ROMAN"|"EUROPEAN_HERBALISM"|"MODERN_SCIENCE"|"OTHER";
    ...
    <Card className="h-full">
      <CardHeader>
        ...
        <CardTitle className="leading-snug">
          <span className="block text-base text-muted-foreground">{entry.title}</span>
      ...
      <CardContent className="space-y-3">
        ...
        {entry.keyDiscoveries && entry.keyDiscoveries.length > 0 && (
          <div>
            ...
            <ul className="list-disc space-y-1 pl-5 text-sm">
              {entry.keyDiscoveries.map((k,i)=>(<li key={i}>{k}</li>))}
            </ul>
          </div>
        )}

        {entry.notablePractitioners && entry.notablePractitioners.length > 0 && (
          <div>
            ...
            <ul className="space-y-1 text-sm">
              {entry.notablePractitioners.map((p,i)=>(
                <li key={i}>
                  ...
                  {p.era ? <span className="text-muted-foreground"> ({p.era})</span> : null}
                </li>
              ))}
            </ul>
          </div>
            ...
            <ul className="space-y-1 text-sm">
              {entry.sources.map((s,i)=>(
                <li key={i}>
                  ...
                  {s.author ? <span className="text-muted-foreground"> — {s.author}</span> : null}
        ...
        {entry.tags && entry.tags.length > 0 && (
          <div className="flex flex-wrap gap-2">
            {entry.tags.map((t,i)=>(<Badge key={i} variant="outline">{t}</Badge>))}
          </div>
        )}
      </CardContent>
    </Card>
