...
import { AlertTriangle, Wifi, WifiOff, RefreshCw, AlertCircle, Info, CheckCircle } from "lucide-react";
import { Alert, AlertDescription, AlertTitle } from "./alert";
import { Button } from "./button";
import { cn } from "@/lib/utils";

export type ErrorSeverity = "low" | "medium" | "high" | "critical";
export type ErrorType = "network" | "validation" | "permission" | "server" | "client" | "timeout" | "unknown";
...
export function ErrorAlert({
  ...
  return (
    <Alert variant={errorVariants[error.severity]} className={cn("w-full", className)}>
          ...
          <div className="space-y-2">
            ...
            {error.details && !compact && (
              <details className="text-xs text-muted-foreground">
                ...
                <p className="mt-2 p-2 bg-muted rounded text-xs font-mono">
                  {error.details}
                </p>
              </details>
            )}
...
export const createErrorInfo = {
  ...
  dosageCalculation: (details?: string): ErrorInfo => ({
    ...
    message: "Wystąpił błąd podczas obliczania dawki suplementu. Sprawdź wprowadzone wartości.",
    details,
    timestamp: new Date(),
    retryable: true,
  }),
  ...
  reviewSubmission: (details?: string): ErrorInfo => ({
    ...
    message: "Wystąpił błąd podczas wysyłania opinii. Spróbuj ponownie za chwilę.",
    details,
    timestamp: new Date(),
    retryable: true,
  }),
