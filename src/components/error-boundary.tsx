...
import ErrorAlert, {
	type ErrorInfo as AppErrorInfo,
	createErrorInfo,
} from "./ui/error-alert";
				...
				<div className="flex min-h-[400px] items-center justify-center p-4">
					<Card className="w-full max-w-md">
						<CardHeader className="text-center">
							...
							<CardDescription>
								Wystąpił nieoczekiwany błąd w aplikacji. Możesz spróbować
								ponownie lub wrócić do strony głównej.
							</CardDescription>
						</CardHeader>
						<CardContent className="space-y-4">
							{this.state.errorInfo && this.props.showDetails && (
								<ErrorAlert error={this.state.errorInfo} compact={true} />
							)}
								...
								{this.props.allowRetry &&
									this.state.retryCount < this.maxRetries && (
										...
										>
											...
											Spróbuj ponownie (
											{this.maxRetries - this.state.retryCount} pozostało)
										</Button>
									)}
							...
							{this.state.retryCount >= this.maxRetries && (
								<p className="text-center text-muted-foreground text-sm">
									Maksymalna liczba prób została przekroczona. Odśwież stronę
...
export const SupplementErrorBoundary: React.FC<{ children: ReactNode }> = ({
	children,
}) => (
	<ErrorBoundary
...
export const CalculatorErrorBoundary: React.FC<{ children: ReactNode }> = ({
	children,
}) => (
	<ErrorBoundary
		onError={(error, errorInfo) => {
