...
export function AdvancedTableFilters({
	...
	const [savedPresets, setSavedPresets] =
		useState<FilterPreset[]>(FILTER_PRESETS);
	...
	const handleCategoryChange = (categories: string[]) => {
		onFilterChange({
			...
			category:
				categories.length > 0
					? (categories as SupplementCategory[])
					: undefined,
		});
	};
	...
	const handleEvidenceLevelChange = (levels: string[]) => {
		onFilterChange({
			...filterConfig,
			evidenceLevel:
				levels.length > 0 ? (levels as EvidenceLevel[]) : undefined,
		});
	};
			...
			<CardContent>
				<Tabs value={activeTab} onValueChange={setActiveTab}>
					...
					<TabsContent value="basic" className="space-y-4">
						{/* Search */}
						<div>
							<Label className="font-medium text-sm">Wyszukiwanie</Label>
								...
								{[
									...
									>
										<Checkbox
											id={category.value}
											checked={
												filterConfig.category?.includes(
								...
								{[
									...
									>
										<Checkbox
											id={level.value}
											checked={
												filterConfig.evidenceLevel?.includes(
													level.value as EvidenceLevel,
												) || false
											}
											onCheckedChange={(checked) => {
												const currentLevels = filterConfig.evidenceLevel || [];
												if (checked) {
													handleEvidenceLevelChange([
														...currentLevels,
														level.value,
													]);
												} else {
							...
							<div className="space-y-6 pr-4">
								...
								<div>
									...
									<div className="space-y-2">
										<div className="flex items-center justify-between">
											<Label className="text-sm">
												Tylko bezpieczne suplementy
											</Label>
											<Switch />
										</div>
										<div className="flex items-center justify-between">
											<Label className="text-sm">
												Pokaż tylko z efektami ubocznymi
											</Label>
											<Switch />
										</div>
									</div>
								...
								<div>
									...
									<div className="space-y-2">
										<div className="flex items-center justify-between">
											<Label className="text-sm">
												Tylko suplementy bez poważnych interakcji
											</Label>
											<Switch />
										</div>
										<div className="flex items-center justify-between">
											<Label className="text-sm">
												Uwzględnij interakcje z lekami
											</Label>
											<Switch defaultChecked />
										</div>
									</div>
										...
										<div
											...
											className={`cursor-pointer rounded-lg border p-3 transition-colors ${
												...
												JSON.stringify(sortConfig) ===
													JSON.stringify(preset.sortConfig)
