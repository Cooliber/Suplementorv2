	...
	switch (action.type) {
		...
		case "TOGGLE_COLUMN":
			return {
				...
				customization: {
					...state.customization,
					visibleColumns: state.customization.visibleColumns.includes(
						action.payload,
					)
						? state.customization.visibleColumns.filter(
		...
		case "TOGGLE_SUPPLEMENT": {
			const isSelected = state.selection.selectedSupplements.includes(
				action.payload,
			);
			return {
				...state,
				selection: {
					...
					selectedSupplements: isSelected
						? state.selection.selectedSupplements.filter(
...
export function SupplementComparisonTable({
	...
	const sortedAndFilteredSupplements = useMemo(() => {
		...
		if (
			state.filterConfig.evidenceLevel &&
			state.filterConfig.evidenceLevel.length > 0
		) {
			filtered = filtered.filter((supplement) =>
	...
	const toggleSupplement = useCallback(
		(id: string) => {
			if (
				state.selection.selectedSupplements.length >= maxSelection &&
	...
	const exportData = useCallback(
		(options: ExportOptions) => {
			...
			try {
				...
				onExport?.(
					sortedAndFilteredSupplements.map(formatSupplementForComparison),
					options.format,
				);
			} catch (error) {
						...
						<div className="flex flex-wrap gap-2">
									...
									<div className="p-4">
										<AdvancedTableFilters
											...
											onFilterChange={(config) =>
												dispatch({ type: "SET_FILTER", payload: config })
											}
											onSortChange={(config) =>
							...
							<DropdownMenu>
								...
								<DropdownMenuContent className="w-56">
									{DEFAULT_COLUMNS.map((column) => (
										<DropdownMenuCheckboxItem
											key={column.key}
											checked={state.customization.visibleColumns.includes(
												column.key,
											)}
							...
							{showCustomization && (
								<Sheet
									...
									onOpenChange={setShowCustomizationPanel}
								>
							...
							{showExport && (
								<DropdownMenu>
									...
									<DropdownMenuContent>
										<DropdownMenuItem
											onClick={() =>
												exportData({ format: "json", includeAllColumns: true })
											}
										>
											Eksportuj jako JSON
										</DropdownMenuItem>
										<DropdownMenuItem
											onClick={() =>
												exportData({ format: "csv", includeAllColumns: true })
											}
										>
											Eksportuj jako CSV
										</DropdownMenuItem>
				...
				<Card>
					<CardContent className="pt-4">
						<div className="flex items-center gap-2 text-muted-foreground text-sm">
							<span>
								Wybrane suplementy ({state.selection.selectedSupplements.length}
								/{maxSelection}):
							</span>
									...
									<div className="flex flex-wrap gap-2">
												...
												<div className="p-2">
													...
													>
														...
														<SelectContent>
															...
															<SelectItem value="NOOTROPIC">
																Nootropiki
															</SelectItem>
															<SelectItem value="HERB">Zioła</SelectItem>
															<SelectItem value="ADAPTOGEN">
																Adaptogeny
															</SelectItem>
														</SelectContent>
													</Select>
										...
										{state.selection.selectedSupplements.length > 0 && (
											...
											>
												<X className="mr-2 h-4 w-4" />
												Wyczyść wybór (
												{state.selection.selectedSupplements.length})
											</Button>
										)}
									</div>
								</div>
						...
						<Card>
							<CardContent className="py-8 text-center">
								<p className="text-muted-foreground">
									Brak suplementów do wyświetlenia. Zmień filtry lub
									wyszukiwanie.
								</p>
							</CardContent>
						</Card>
				...
				<Card>
					<CardContent className="p-0">
						<div className="relative overflow-x-auto">
							<Table
								className={state.customization.showBorders ? "border" : ""}
							>
								<TableHeader>
									<TableRow>
								...
								<TableBody>
									{sortedAndFilteredSupplements.map((supplement) => (
										<TableRow
											key={supplement.id}
											className={`${
												state.selection.selectedSupplements.includes(
													supplement.id,
												)
													? "bg-muted/50"
													: ""
