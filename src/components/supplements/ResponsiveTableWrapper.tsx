			...
			case "researchCount":
				return (
					<div className="text-center">
						<div className="font-medium">
							{supplement.researchStudies.length}
						</div>
				...
				<div className="space-y-3">
					...
					<div className="flex items-start justify-between">
						<div className="flex-1">
							{getColumnValue(
								visibleColumns.find((col) => col.key === "name")!,
							)}
						</div>
						<div className="flex flex-col items-end gap-1">
							{getColumnValue(
								visibleColumns.find((col) => col.key === "category")!,
							)}
							{getColumnValue(
								visibleColumns.find((col) => col.key === "evidenceLevel")!,
							)}
						</div>
					</div>
						...
						{visibleColumns
							...
							.map((column) => (
								<div key={column.key}>
									...
									<div className="mt-1">{getColumnValue(column)}</div>
								</div>
							))}
					</div>
													...
													return (
														...
														>
															...
															<div className="text-right">
																{column.render
																	? column.render(value, supplement)
																	: value}
															</div>
														</div>
													);
