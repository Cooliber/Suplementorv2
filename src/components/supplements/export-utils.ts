...
function exportToJSON(data: any[], options: ExportOptions): void {
  ...
  downloadFile(jsonData, options.filename || "supplements.json", "application/json");
}
...
function exportToExcel(data: any[], options: ExportOptions): void {
  ...
  downloadFile(csvContent, options.filename || "supplements.tsv", "text/tab-separated-values");
}
...
function generatePDFHTML(data: any[], headers: string[], options: ExportOptions): string {
  ...
  switch (header) {
    ...
    case "clinicalApplications":
      if (Array.isArray(value) && value.length > 0) {
        return value.slice(0, 2).map((app: any) =>
          `${app.condition} (${app.polishCondition})`
        ).join(", ");
      }
      return "-";
    case "sideEffects":
      if (Array.isArray(value) && value.length > 0) {
        const commonEffects = value.filter((se: any) => se.frequency === "common");
        return commonEffects.slice(0, 2).map((se: any) => se.effect).join(", ");
      }
      return "Minimalne";
    case "interactions":
      if (Array.isArray(value) && value.length > 0) {
        const severe = value.filter((i: any) => i.severity === "severe").length;
        const moderate = value.filter((i: any) => i.severity === "moderate").length;
        let result = "";
        if (severe > 0) result += `${severe} poważne`;
        if (moderate > 0) result += `${result ? ", " : ""}${moderate} umiarkowane`;
...
export function getAvailableExportFormats(): Array<{ value: ExportOptions["format"]; label: string; description: string }> {
  return [
    {
      value: "csv",
      label: "CSV",
      description: "Plik wartości rozdzielanych przecinkami, kompatybilny z Excel"
    },
    {
      value: "excel",
      label: "Excel (TSV)",
...
export function validateExportOptions(options: ExportOptions): { valid: boolean; errors: string[] } {
  ...
  if (options.format === "csv" || options.format === "excel") {
    if (!options.includeAllColumns && (!options.selectedColumns || options.selectedColumns.length === 0)) {
      errors.push("Wybierz kolumny do eksportu lub zaznacz 'Uwzględnij wszystkie kolumny'");
    }
  }
