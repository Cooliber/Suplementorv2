...
export function ComparisonView({
  ...
  const getComparisonIndicator = (values: any[]) => {
    ...
    // For evidence levels
    if (typeof firstValue === "string" && ["STRONG", "MODERATE", "WEAK", "INSUFFICIENT", "CONFLICTING"].includes(firstValue)) {
      const levelOrder = { STRONG: 4, MODERATE: 3, WEAK: 2, INSUFFICIENT: 1, CONFLICTING: 0 };
      const firstLevel = levelOrder[firstValue as keyof typeof levelOrder];
      const maxLevel = Math.max(...values.map(v => levelOrder[v as keyof typeof levelOrder]));
      ...
      <CardContent>
        <ScrollArea className="h-[600px]">
          <div className="space-y-6">
            {/* Supplement Headers */}
            <div className="grid gap-4" style={{ gridTemplateColumns: `200px repeat(${selectedSupplements.length}, 1fr)` }}>
              <div></div>
              {selectedSupplements.map((supplement) => (
                <Card key={supplement.id} className="p-3">
                  <div className="text-center">
                    ...
                    <div className="text-xs text-muted-foreground">{supplement.polishName}</div>
            ...
            {comparisonData.map((row, rowIndex) => (
              <div key={rowIndex}>
                <div className="grid gap-4 mb-3" style={{ gridTemplateColumns: `200px repeat(${selectedSupplements.length}, 1fr)` }}>
                ...
                {rowIndex < comparisonData.length - 1 && <Separator className="my-4" />}
              </div>
            ))}

            {/* Detailed Applications */}
            <div>
              <h4 className="font-medium mb-3">Zastosowania kliniczne</h4>
              <div className="grid gap-4" style={{ gridTemplateColumns: `200px repeat(${selectedSupplements.length}, 1fr)` }}>
                <div></div>
                {selectedSupplements.map((supplement) => (
                  <div key={supplement.id} className="space-y-2">
                    {supplement.clinicalApplications.slice(0, 3).map((app, idx) => (
                      <div key={idx} className="text-xs p-2 bg-muted rounded">
                        <div className="font-medium">{app.condition}</div>
                        <div className="text-muted-foreground">{app.polishCondition}</div>
                        <div className="text-muted-foreground">{app.recommendedDose}</div>
                      </div>
                    ))}
                  </div>
                ))}
            ...
            <div>
              <h4 className="font-medium mb-3">Mechanizmy działania</h4>
              <div className="grid gap-4" style={{ gridTemplateColumns: `200px repeat(${selectedSupplements.length}, 1fr)` }}>
                <div></div>
                {selectedSupplements.map((supplement) => (
                  <div key={supplement.id} className="space-y-1">
                    {supplement.mechanisms.slice(0, 2).map((mechanism, idx) => (
                      <div key={idx} className="text-xs p-2 bg-blue-50 rounded">
                        ...
                        <div className="text-muted-foreground">{mechanism.polishPathway}</div>
                      </div>
                    ))}
                  </div>
                ))}
