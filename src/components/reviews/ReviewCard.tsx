...
import { Star, ThumbsUp, ThumbsDown, Shield, ShoppingCart, TrendingUp, Award, Clock, Pill } from "lucide-react";
...
export function ReviewCard({
						...
						<div className="space-y-2">
							{review.pros.map((pro, index) => (
								<div key={index} className="flex items-start gap-2 p-2 bg-green-50 rounded-lg border border-green-100">
									...
									<span className="text-sm text-green-700 leading-relaxed">{pro}</span>
								</div>
							))}
						</div>
					</div>
						...
						<div className="space-y-2">
							{review.cons.map((con, index) => (
								<div key={index} className="flex items-start gap-2 p-2 bg-red-50 rounded-lg border border-red-100">
									...
									<span className="text-sm text-red-700 leading-relaxed">{con}</span>
								</div>
							))}
						</div>
					</div>
	...
	const renderDetailedRatings = () => {
		if (!review.effectiveness && !review.valueForMoney && !review.easeOfUse) return null;

		const ratings = [
			{ key: 'effectiveness', label: 'Skuteczność', value: review.effectiveness, icon: Award, color: 'blue' },
			{ key: 'valueForMoney', label: 'Stosunek jakość/cena', value: review.valueForMoney, icon: ShoppingCart, color: 'green' },
			{ key: 'easeOfUse', label: 'Łatwość stosowania', value: review.easeOfUse, icon: Pill, color: 'purple' },
		].filter(r => r.value);
	...
	return (
		<TooltipProvider>
			<Card className={cn(
				"w-full transition-all duration-300 hover:shadow-lg hover:shadow-primary/5 border-l-4",
						...
						<div className="flex items-start space-x-4 flex-1">
							...
							<div className="flex-1 min-w-0">
								<div className="flex items-center gap-3 mb-2">
									...
									<div className="flex items-center gap-2">
										<div className="flex items-center gap-1 px-2 py-1 bg-yellow-50 rounded-full border">
								...
								<Tooltip>
									<TooltipTrigger asChild>
										<Button
											...
											disabled={isVoting}
											className="h-9 px-3 hover:bg-green-50 hover:text-green-700 transition-colors"
										>
								...
								<Tooltip>
									<TooltipTrigger asChild>
										<Button
											...
											disabled={isVoting}
											className="h-9 px-3 hover:bg-red-50 hover:text-red-700 transition-colors"
										>
