...
const KnowledgeAtom: React.FC<KnowledgeAtomProps> = ({
	...
	if (compact) {
		return (
			<TooltipProvider>
				<Card className="group hover:-translate-y-0.5 cursor-pointer transition-all hover:shadow-md">
					<CardContent className="p-4">
						<div className="flex items-start gap-3">
							<div className="flex-shrink-0">{getTypeIcon(atom.type)}</div>
							<div className="min-w-0 flex-1">
								<div className="mb-2 flex items-center gap-2">
									...
									<Tooltip>
										<TooltipTrigger>
											<Badge
												variant="outline"
												className={getEvidenceColor(atom.evidenceLevel)}
											>
												{atom.evidenceLevel}
											</Badge>
										</TooltipTrigger>
								...
								<div className="mt-2 flex items-center gap-2">
									<Badge
										variant="secondary"
										className={`text-xs ${getCategoryColor(atom.category)}`}
									>
										{atom.category}
									</Badge>
		...
		<Card className="group transition-all hover:shadow-lg">
			<CardHeader className="pb-4">
				<div className="flex items-start justify-between">
					<div className="flex flex-1 items-start gap-3">
						<div className="mt-1 flex-shrink-0">{getTypeIcon(atom.type)}</div>
						<div className="min-w-0 flex-1">
							<div className="mb-2 flex items-center gap-2">
			...
			<CardContent className="space-y-4">
				<div className="prose prose-sm max-w-none">
					<p className="text-gray-700 leading-relaxed">{isContent}</p>
				</div>
								...
								{atom.references.slice(0, 3).map((reference, index) => (
									<div key={index} className="group flex items-start gap-2">
										...
										<Tooltip>
											<TooltipTrigger asChild>
												<button
													className="flex-1 text-left text-left text-blue-600 text-sm hover:text-blue-800 hover:underline"
