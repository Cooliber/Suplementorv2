...
export function useCachedSupplements(
	...
	const checkCacheStatus = useCallback(() => {
		...
		if (query && Object.keys(query).length > 0) {
			const status = cachingService.getCacheStatus(
				`supplements:${JSON.stringify(query)}`,
			);
...
export function useCachedSupplementSearch(
	options: CachedSupplementOptions = {},
) {
	const {
	...
	const checkCacheStatus = useCallback(() => {
		...
		if (searchResult.searchResults.length > 0) {
			const status = cachingService.getCacheStatus(
				`search:${searchResult.searchResults.length}`,
			);
			setCacheStatus(status);
			onSync?.(status);
		}
	...
	return {
		...
		pendingChanges,
		refetch: () =>
			searchResult.search(searchResult.searchResults.length > 0 ? "last" : ""),
		refreshCache,
		clearCache,
		search,
...
export class CachedSupplementService {
	...
	async getSupplements(query: any = {}): Promise<any[]> {
		...
		try {
			...
			const { default: SupplementService } = await import(
				"@/lib/services/supplement-service"
			);
			...
			// Cache the result
			this.cachingService.set(cacheKey, result, {
				ttl: 5 * 60 * 1000,
				source: "network",
			});
