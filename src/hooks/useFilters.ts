		...
		case "CLEAR_SECTION":
			...
			if (Array.isArray(currentValue)) {
				if (currentValue.length === 2 && typeof currentValue[0] === "number") {
					...
					const defaultRange: [number, number] = [0, action.section === "priceRange" ? 1000 : 10];
...
const generateUrlState = (filters: FilterState): UrlFilterState => {
	...
	if (filters.evidenceLevels.length > 0) urlState.e = filters.evidenceLevels.join(",");
	...
	if (filters.activeCompounds.length > 0) urlState.ac = filters.activeCompounds.join(",");
	if (filters.clinicalConditions.length > 0) urlState.cc = filters.clinicalConditions.join(",");
	if (filters.mechanisms.length > 0) urlState.m = filters.mechanisms.join(",");
	if (filters.sideEffects.length > 0) urlState.se = filters.sideEffects.join(",");
...
export function useFilters(initialFilters?: Partial<FilterState>): UseFiltersReturn {
	...
	useEffect(() => {
		if (typeof window !== "undefined") {
			...
			Object.keys(params).forEach(key => {
				if (key.startsWith("q") || key.startsWith("c") || key.startsWith("e") ||
					key.startsWith("p") || key.startsWith("d") || key.startsWith("r") ||
					key.startsWith("s") || key.startsWith("a") || key.startsWith("m") ||
					key.startsWith("t") || key.startsWith("h") || key.startsWith("o") ||
					key.startsWith("n") || key.startsWith("cm")) {
					params.delete(key);
				}
			});
	...
	const activeFilterCount = useCallback(() => {
		...
		if (filters.dosageRange[0] !== 0 || filters.dosageRange[1] !== 5000) count++;
		if (filters.ratingRange[0] !== 0 || filters.ratingRange[1] !== 5) count++;
		if (filters.safetyRating[0] !== 0 || filters.safetyRating[1] !== 10) count++;
