...
import { ErrorInfo, createErrorInfo } from "@/components/ui/error-alert";
...
export function useErrorHandler(options: UseErrorHandlerOptions = {}) {
  ...
  const retryLastError = useCallback(async () => {
    ...
    try {
      ...
      addError(createErrorInfo.server(
  ...
  const handleSupplementError = useCallback((supplementName?: string, details?: string) => {
  ...
  const handleDosageError = useCallback((details?: string) => {
    const errorInfo = createErrorInfo.dosageCalculation(details);
    addError(errorInfo);
    return errorInfo;
  }, [addError]);

  const handleReviewError = useCallback((details?: string) => {
